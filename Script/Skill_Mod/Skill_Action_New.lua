...
act.FireBall={
    ...
    main 	= 	function(playerid) 
       ...
       local lavaball = 15003;--[[Bullet]]       local maxi = 20;
       for i=1,maxi,2 do 
            local r = math.min((maxi/2)-(i/2),maxi/5);
            local cX,cY,cZ = dx+(math.random(-r,r)/4),dy,dz+(math.random(-r,r)/4);
            
            RUNNER.NEW(function(playerid,x,y,z,cX,cY,cZ)
                local Px,Py,Pz = getPos(playerid);local dx,dy,dz = getDir(playerid);
                local re,projectileID = World:spawnProjectile(playerid,15003 , Px+dx,1+Py+dy,Pz+dz, cX,cY,cZ , 160);
                ...
                RUNNER.ATTACH_Func_Obj(projectileID,function(projectileID,e)
                    ...
                    addEffect(x,y,z,specialEffect.explosion1,1); 
                    playSoundOnPos(x,y,z,soundEffect.explosion,70);
                    dealsDamageButNotHost(playerid,x,y,z,3,3,3,100,1)

                    RUNNER.NEW(function(x,y,z)
                        cancelEffect(x,y,z,specialEffect.explosion1)
                    end,{x,y,z},20)
                ...
                for i=2,40,2 do 
                    RUNNER.NEW(function(projectileID) 
                        local r,objtype = Actor:getObjType(projectileID)
                        --print(" Is a",r,objtype);
                        if(r==0)then 
                            local x,y,z = getPos(projectileID);
                            local ownerPlayerid = RUNNER.Obj_OF(projectileID);
                            dealsDamageButNotHost(ownerPlayerid,x,y,z,2,2,2,2,1);
                            --print(ownerPlayerid,"Dealing damage from ",projectileID,x,y,z)
                        else 
                            RUNNER.Obj_UNREGISTER(projectileID);
                        end 
                    end,{projectileID},i);
                end 
       ...
       for i=1,maxi do 
           ...
           dX,dY,dZ = dX+math.random(-3,3),dY+math.random(0,3),dZ+math.random(-3,3);
            ...
            RUNNER.NEW(function(playerid,dX,dY,dZ)
                local re,projectileID = World:spawnProjectile(playerid,15508 , dX,dY,dZ, dX,dY+2,dZ, 40);
