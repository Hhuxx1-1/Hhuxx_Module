...
RUNNER.CALL                     = function(f,args)              table.insert(RUNNER.EVENT,{f=f,args=args}); end ; RUNNER.EXECUTE_DELAYED_EVENT    = function(idEvent)    local toExecute = RUNNER.DELAYED_EVENT[idEvent] ; if toExecute.delay > 0 then RUNNER.DELAYED_EVENT[idEvent].delay = toExecute.delay - 1;    else  local success, result = pcall(function() return toExecute.f(unpack(toExecute.args)) end) if success then  table.remove(RUNNER.DELAYED_EVENT, idEvent); return result else print("Error executing :", result);table.remove(RUNNER.DELAYED_EVENT, idEvent); end end end ; RUNNER.EXECUTE                  = function(idEvent)    local toExecute = RUNNER.EVENT[idEvent];    local result = toExecute.f(unpack(toExecute.args));    table.remove(RUNNER.EVENT,idEvent);    return result; end ; ScriptSupportEvent:registerEvent("Game.RunTime",function(e) if(RUNNER.LENGTH() > 0 )then  for i,a in pairs(RUNNER.EVENT) do RUNNER.EXECUTE(i); end end end);ScriptSupportEvent:registerEvent("Game.RunTime",function(e) if(RUNNER.DELAY_LENGTH() > 0 )then for i,a in pairs(RUNNER.DELAYED_EVENT) do RUNNER.EXECUTE_DELAYED_EVENT(i);end end end);
...
RUNNER.Obj_OF                   = function(objectid)            return RUNNER.OBJECT[objectid]; end;
