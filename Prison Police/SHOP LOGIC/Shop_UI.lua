...
local function loadIntoSlot(i,data,playerid)
    ...
    Customui:setText(playerid,uiid,ThisUI.PriceSlot_Txt[i],GLOBAL_CURRENCY:PrettyDisplay(data.price));
...
local function getShopList(npc)
    if type(npc) == "table" then 
        local Shoplist = GLOBAL_SHOP.VSHOP[npc.VSHOP];
...
local function LoadDesc(i,playerid)
    ...
    local iex;
    if type(npc) == "table" then
        iex = dynamicNpc_Indices[npc.VSHOP][i];
    else 
        iex = dynamicNpc_Indices[npc][i];
    end 
...
local function LoadShopDataIntoUI(playerid,npc)
    ...
    if NPC_Data then 
        ...
        LoadItemShoplist(ItemShop,npc.VSHOP,npc.VSHOP,playerid)
    end 
end
...
local function BuyCurrentSelectedItem(playerid)
    ...
    if stock2Buy <= ItemShop[iex].stock then
        ...
        if r == 0 then 
            ...
            if type(npc) == "table" then
                GLOBAL_SHOP.VSHOP[npc.VSHOP][iex].stock=ItemShop[iex].stock - stock2Buy ;
                print(GLOBAL_SHOP.VSHOP);
            else
